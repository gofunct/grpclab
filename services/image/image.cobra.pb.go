...
var _DefaultImageServiceClientCommandConfig = _NewImageServiceClientCommandConfig()

type _ImageServiceClientCommandConfig struct {
...
func _NewImageServiceClientCommandConfig() *_ImageServiceClientCommandConfig {
	c := &_ImageServiceClientCommandConfig{}
	envconfig.Process("", c)
	return c
}

func (o *_ImageServiceClientCommandConfig) AddFlags(fs *pflag.FlagSet) {
...
var ImageServiceClientCommand = &cobra.Command{
	Use: "imageservice",
}

func _DialImageService() (*grpc.ClientConn, ImageServiceClient, error) {
	cfg := _DefaultImageServiceClientCommandConfig
	...
	return conn, NewImageServiceClient(conn), nil
}

type _ImageServiceRoundTripFunc func(cli ImageServiceClient, in iocodec.Decoder, out iocodec.Encoder) error

func _ImageServiceRoundTrip(sample interface{}, fn _ImageServiceRoundTripFunc) error {
	cfg := _DefaultImageServiceClientCommandConfig
	...
	conn, client, err := _DialImageService()
	if err != nil {
		return err
	}
	defer conn.Close()
...
var _ImageServiceStoreClientCommand = &cobra.Command{
	Use:  "store",
...
Authenticate using the Authorization header (requires transport security):
	...
	Run: func(cmd *cobra.Command, args []string) {
		var v ImageStoreRequest
		err := _ImageServiceRoundTrip(v, func(cli ImageServiceClient, in iocodec.Decoder, out iocodec.Encoder) error {
			...
			stream, err := cli.Store(context.Background(), &v)
...
func init() {
	ImageServiceClientCommand.AddCommand(_ImageServiceStoreClientCommand)
	_DefaultImageServiceClientCommandConfig.AddFlags(_ImageServiceStoreClientCommand.Flags())
}

var _ImageServiceStoreSyncClientCommand = &cobra.Command{
	Use:  "storesync",
	Long: "StoreSync client\n\nYou can use environment variables with the same name of the command flags.\nAll caps and s/-/_, e.g. SERVER_ADDR.",
	Example: `
Save a sample request to a file (or refer to your protobuf descriptor to create one):
	storesync -p > req.json

Submit request using file:
	storesync -f req.json

Authenticate using the Authorization header (requires transport security):
	export AUTH_TOKEN=your_access_token
	export SERVER_ADDR=api.example.com:443
	echo '{json}' | storesync --tls`,
	Run: func(cmd *cobra.Command, args []string) {
		var v ImageStoreRequest
		err := _ImageServiceRoundTrip(v, func(cli ImageServiceClient, in iocodec.Decoder, out iocodec.Encoder) error {
			...
			resp, err := cli.StoreSync(context.Background(), &v)
...
func init() {
	ImageServiceClientCommand.AddCommand(_ImageServiceStoreSyncClientCommand)
	_DefaultImageServiceClientCommandConfig.AddFlags(_ImageServiceStoreSyncClientCommand.Flags())
}

var _ImageServiceDeleteClientCommand = &cobra.Command{
	Use:  "delete",
	Long: "Delete client\n\nYou can use environment variables with the same name of the command flags.\nAll caps and s/-/_, e.g. SERVER_ADDR.",
	Example: `
Save a sample request to a file (or refer to your protobuf descriptor to create one):
...
func init() {
	ImageServiceClientCommand.AddCommand(_ImageServiceDeleteClientCommand)
	_DefaultImageServiceClientCommandConfig.AddFlags(_ImageServiceDeleteClientCommand.Flags())
}

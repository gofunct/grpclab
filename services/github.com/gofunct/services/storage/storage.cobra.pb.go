...
It has these top-level commands:
	CloudStorageServiceClientCommand
*/
...
var _DefaultCloudStorageServiceClientCommandConfig = _NewCloudStorageServiceClientCommandConfig()

type _CloudStorageServiceClientCommandConfig struct {
...
func _NewCloudStorageServiceClientCommandConfig() *_CloudStorageServiceClientCommandConfig {
	c := &_CloudStorageServiceClientCommandConfig{}
	envconfig.Process("", c)
	return c
}

func (o *_CloudStorageServiceClientCommandConfig) AddFlags(fs *pflag.FlagSet) {
...
var CloudStorageServiceClientCommand = &cobra.Command{
	Use: "cloudstorageservice",
}

func _DialCloudStorageService() (*grpc.ClientConn, CloudStorageServiceClient, error) {
	cfg := _DefaultCloudStorageServiceClientCommandConfig
	...
	return conn, NewCloudStorageServiceClient(conn), nil
}

type _CloudStorageServiceRoundTripFunc func(cli CloudStorageServiceClient, in iocodec.Decoder, out iocodec.Encoder) error

func _CloudStorageServiceRoundTrip(sample interface{}, fn _CloudStorageServiceRoundTripFunc) error {
	cfg := _DefaultCloudStorageServiceClientCommandConfig
	...
	conn, client, err := _DialCloudStorageService()
	if err != nil {
		return err
	}
	defer conn.Close()
...
var _CloudStorageServiceStoreClientCommand = &cobra.Command{
	Use:  "store",
...
Authenticate using the Authorization header (requires transport security):
	...
	Run: func(cmd *cobra.Command, args []string) {
		var v StoreRequest
		err := _CloudStorageServiceRoundTrip(v, func(cli CloudStorageServiceClient, in iocodec.Decoder, out iocodec.Encoder) error {
			...
			resp, err := cli.Store(context.Background(), &v)
...
func init() {
	CloudStorageServiceClientCommand.AddCommand(_CloudStorageServiceStoreClientCommand)
	_DefaultCloudStorageServiceClientCommandConfig.AddFlags(_CloudStorageServiceStoreClientCommand.Flags())
}

var _CloudStorageServiceDeleteClientCommand = &cobra.Command{
	Use:  "delete",
	Long: "Delete client\n\nYou can use environment variables with the same name of the command flags.\nAll caps and s/-/_, e.g. SERVER_ADDR.",
	Example: `
Save a sample request to a file (or refer to your protobuf descriptor to create one):
...
func init() {
	CloudStorageServiceClientCommand.AddCommand(_CloudStorageServiceDeleteClientCommand)
	_DefaultCloudStorageServiceClientCommandConfig.AddFlags(_CloudStorageServiceDeleteClientCommand.Flags())
}

...
var _DefaultEmailServiceClientCommandConfig = _NewEmailServiceClientCommandConfig()

type _EmailServiceClientCommandConfig struct {
...
func _NewEmailServiceClientCommandConfig() *_EmailServiceClientCommandConfig {
	c := &_EmailServiceClientCommandConfig{}
	envconfig.Process("", c)
	return c
}

func (o *_EmailServiceClientCommandConfig) AddFlags(fs *pflag.FlagSet) {
...
var EmailServiceClientCommand = &cobra.Command{
	Use: "emailservice",
}

func _DialEmailService() (*grpc.ClientConn, EmailServiceClient, error) {
	cfg := _DefaultEmailServiceClientCommandConfig
	...
	return conn, NewEmailServiceClient(conn), nil
}

type _EmailServiceRoundTripFunc func(cli EmailServiceClient, in iocodec.Decoder, out iocodec.Encoder) error

func _EmailServiceRoundTrip(sample interface{}, fn _EmailServiceRoundTripFunc) error {
	cfg := _DefaultEmailServiceClientCommandConfig
	...
	conn, client, err := _DialEmailService()
	if err != nil {
		return err
	}
	defer conn.Close()
...
var _EmailServiceSendClientCommand = &cobra.Command{
	Use:  "send",
...
Authenticate using the Authorization header (requires transport security):
	...
	Run: func(cmd *cobra.Command, args []string) {
		var v Email
		err := _EmailServiceRoundTrip(v, func(cli EmailServiceClient, in iocodec.Decoder, out iocodec.Encoder) error {
			...
			resp, err := cli.Send(context.Background(), &v)
...
func init() {
	EmailServiceClientCommand.AddCommand(_EmailServiceSendClientCommand)
	_DefaultEmailServiceClientCommandConfig.AddFlags(_EmailServiceSendClientCommand.Flags())
}
